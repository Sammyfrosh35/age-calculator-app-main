...
//         let totalDaysPM = new Date(thisYear, (thisMonth - 1), 0).getDate(); 1
...
function calc() {
    ...
    let thisMonth = today.getMonth() + 1;
    let thisDay = today.getDate();
    let totalDaysPM = new Date(thisYear, thisMonth, 0).getDate();
    let beforeRDay = totalDaysPM - thisDay;
    let beforeRDay0 = beforeRDay + thisDay;
    ...
    // Check if the year is equal to the current year
    if (year == thisYear) {
        yearR.innerText = 0;
    } else if (year < thisYear) {
        yearR.innerText = thisYear - year;
    } else {
        document.querySelector(".invalid__string__year").style.display = "block";
        refresh();
        return;
    ...
    // Check if the month is in the past or future
    if (month < thisMonth) {
        monthR.innerText = thisMonth - month;
    } else if (month > thisMonth) {
        monthR.innerText = totalMonths - (month - thisMonth);
    } else {
        monthR.innerText = 0;
    }
    ...
    if (day < thisDay) {
        dayR.innerText = thisDay - day;
    } else if (day > thisDay) {
        dayR.innerText = beforeRDay0 - day;
    } else {
        dayR.innerText = 0;
    }

    // Hide empty string labels and refresh function
    for (var i = 0; i < 3; i++) {
        document.querySelectorAll(".empty__string__text")[i].style.display = "none";
        document.querySelectorAll("label")[i].classList.remove("empty__string-label");
        document.querySelectorAll("input")[i].classList.remove("empty__string-input");
